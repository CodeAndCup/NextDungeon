...
| `/dungeon debug toggle`                        | Toggle debug mode on/off at runtime                  |
| `/dungeon debug setlogbroadcast <type>`        | Change the debug log output type (`CONSOLE`, `IN_GAME`, `BOTH`) |
| `/dungeon debug floor <dungeonId> <floorId>`   | Print detailed debug info for a specific floor       |
| `/dungeon debug trigger <dungeonId> <floorId>` | Print trigger debug info for a specific floor        |

***

## Console Commands

Some commands are available from the server console only.

<table><thead><tr><th width="415">Command</th><th>Description</th></tr></thead><tbody><tr><td><code>/dungeon console openmenu &#x3C;dungeonId> &#x3C;player></code></td><td>Open the menu of the indicated dungeon for the indicated player.</td></tr></tbody></table>
...
| _(none required)_    | All `/dungeon` player commands are accessible to every player | Everyone |
